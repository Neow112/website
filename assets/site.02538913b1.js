(() => {
  ...
  function changeLanguage(value) {
    ...
    try { localStorage.setItem('sapfir-wow-language', lang); } catch {}
  }

  // Intro: keep the rest of the page inert until the branded intro is finished.
  (() => {
    ...
    setTimeout(() => endIntro(false), 3100);
  })();

  $$('.language-native').forEach(input => input.addEventListener('change', () => {
    if (input.checked) changeLanguage(input.value);
  }));
  ...
  document.addEventListener('copy', event => { if (!selectionIsPhoneOnly()) event.preventDefault(); }, true);
  ...
  async function copyPhone(element) {
    ...
    try {
      ...
      helper.select();
      try { document.execCommand('copy'); showToast(copiedMessage()); } catch {}
      helper.remove();
    }
  }
  let hold = null;
  ...
  let savedLanguage;
  try { savedLanguage = localStorage.getItem('sapfir-wow-language'); } catch {}
  changeLanguage(supportedLanguages.includes(savedLanguage) ? savedLanguage : 'ru');
  syncGallery();
  paintPage();
})();
